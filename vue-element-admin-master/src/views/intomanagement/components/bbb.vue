       ...
       <!-- 信息选择查看部分 -->   
           ...
           <div class="showBoxs" v-show="isShow1">
               ...
               <div class="items"><div class="titles">营业执照：</div>
                   <span >
                     <img class="logoImg" v-show="msg.license_image.length" :src="msg.license_image" alt="">
               ...
               <div class="items"><div class="titles">法人身份证扫描件：</div>
                <span>
                     <img class="logoImg1"  v-for="(p, index) in msg.legal_id_card_img" :key="index"  @click="deleted(index)" v-show="msg.legal_id_card_img.length" :src="p" alt="">
           ...
           <div class="showBoxs" v-show="isShow2">
               ...
               <div class="items"><div class="titles">运营协议：</div>
                   <span>
                     <img class="logoImg" v-show="msg.operator_agreement.length" :src="msg.operator_agreement" alt="">
                ...
                <div class="items"><div class="titles">企业logo：</div>
                   <span>
                     <img class="logoImg" v-show="msg.channel_logo.length" :src="msg.channel_logo" alt="">
               ...
               <span class="showImg">
                     <img class="logoImg1 l1"  @click="deleted1(index)"  v-for="(p, index) in msg.supplement" :key="index" v-show="msg.supplement.length" :src="p" alt="">
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      alert("渠道商进件添加");
      var param = new FormData(); // 创建form

      param.append("bank", this.msg.bank);
      param.append("name", this.msg.name);
      param.append("license_no", this.msg.license_no);
      param.append("date", this.msg.date);
      param.append("city", this.msg.city);
      param.append("location", this.msg.location);
      param.append("business_scope", this.msg.business_scope);
      param.append("legal_id_card", this.msg.legal_id_card);
      param.append("legal_name", this.msg.legal_name);
      param.append("operator_name", this.msg.operator_name);
      param.append("operator_mobile", this.msg.operator_mobile);
      param.append("operator_email", this.msg.operator_email);

      param.append("service_tel", this.msg.service_tel);
      param.append("operator_type", this.msg.operator_type);
      param.append("business_type", this.msg.business_type);
      param.append("rate", this.msg.rate);
      param.append("clear_type", this.msg.clear_type);

      param.append("bankcard_name", this.msg.bankcard_name);
      param.append("bank_name", this.msg.bank_name);
      param.append("bank_credit", this.msg.bank_credit);

      param.append("license_image", this.license_image);
      param.append("legal_id_card_front", this.legal_id_card_front);

      param.append("legal_id_card_backend", this.legal_id_card_backend);
      param.append("operator_agreement", this.operator_agreement);
      param.append("channel_logo", this.channel_logo);
      param.append("supplement", this.supplement);
      console.log("====================================");
    ...
    handleFile: function(e) {
      ...
      reader.onload = data => {
        ...
        if ($target.className == "hiddenInput") {
          that.msg.license_image = res.result;
          that.license_image = that.convertBase64UrlToBlob(res.result, "wukai");

          console.log(111);
          console.log(that.license_image);

          // param.append("license_image", aaa); // 通过append向form对象添加数据
          // param.append("name", "吴凯");
          // console.log(aaa);
          // imgUp(param)
          //   .then(res => {
          //     console.log(res);
          ...
          var n = 0;
          if (that.msg.legal_id_card_img.length >= 2) {
            that.msg.legal_id_card_img.splice(0, 1);
            console.log(++n);
          }
          that.msg.legal_id_card_img.push(res.result);
          if (that.msg.legal_id_card_img.length == 1) {
            that.legal_id_card_front = files;
            console.log(11);
            console.log(that.legal_id_card_front);
          } else if (that.msg.legal_id_card_img.length == 2) {
            that.legal_id_card_backend = files;
            console.log(22);
            console.log(that.legal_id_card_backend);
          }

          // var param = new FormData(); // 创建form对象
          // param.append("img1", file); // 通过append向form对象添加数据
          // param.append("img2", file);
          // param.append("name", "身份证");
          // var abb = this.msg.license_image.split(",")[1];
          // var data = { license_image: abb, name: "吴凯" };
          // imgUp(param)
          //   .then(res => {
          //     console.log(res);
          //   })
          ...
          that.msg.operator_agreement = res.result;
          that.operator_agreement = that.convertBase64UrlToBlob(
            res.result,
            "wukai"
          );
        } else if ($target.className == "hiddenInput3") {
          that.msg.channel_logo = res.result;
          that.channel_logo = that.convertBase64UrlToBlob(res.result, "wukai");
        } else if ($target.className == "hiddenInput4") {
          var n = 0;
          if (this.msg.supplement.length >= 8) {
            that.msg.supplement.splice(0, 1);
            that.supplement.splice(0, 1);
            console.log(++n);
            // console.log(res.result);
          }
          that.msg.supplement.push(res.result);
          that.supplement.push(
            that.convertBase64UrlToBlob(res.result, "wukai")
          );
        }
      };
